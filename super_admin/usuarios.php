...
if (isset($_GET['eliminar']) && !empty($_GET['eliminar'])) {
    ...
    try {
        ...
        // Verificar si el usuario tiene registros asociados en asistencia
        $stmt = $conn->prepare("SELECT COUNT(*) FROM asistencia WHERE documento = ?");
        $stmt->execute([$id_eliminar]);
        $tiene_asistencias = ($stmt->fetchColumn() > 0);
        
        // Verificar si el usuario ha creado cursos
        $stmt = $conn->prepare("SELECT COUNT(*) FROM cursos WHERE id_creador = ?");
        $stmt->execute([$id_eliminar]);
        $tiene_cursos = ($stmt->fetchColumn() > 0);
        
        if ($tiene_asistencias || $tiene_cursos) {
...
try {
    ...
    if (!empty($busqueda)) {
        $where[] = "(u.nombres LIKE ? OR u.apellidos LIKE ? OR u.email LIKE ? OR u.id LIKE ?)";
        $params[] = "%$busqueda%";
        $params[] = "%$busqueda%";
        $params[] = "%$busqueda%";
        $params[] = "%$busqueda%";
    ...
    $stmt = $conn->prepare("SELECT u.*, e.nombre as empresa_nombre, r.nombre_rol 
                           FROM usuarios u 
                           LEFT JOIN empresa e ON u.id_empresa = e.id 
                           LEFT JOIN rol r ON u.rol = r.id 
                           $whereClause
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if (!empty($usuario['empresa_nombre'])): ?>
                                                    <a href="empresas_ver.php?id=<?php echo $usuario['id_empresa']; ?>">
