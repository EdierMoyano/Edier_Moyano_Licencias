...
if (isset($_GET['eliminar']) && !empty($_GET['eliminar'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM licencias WHERE id_empresa = ?");
...
try {
    ...
    $stmt = $conn->prepare("SELECT e.*, 
                           (SELECT COUNT(*) FROM licencias WHERE id_empresa = e.id) as total_licencias,
                           (SELECT COUNT(*) FROM usuarios WHERE id_empresa = e.id) as total_usuarios
                           FROM empresa e
                           $where
                           ORDER BY e.id DESC
...
<body>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a href="empresas_ver.php?id=<?php echo $empresa['id']; ?>" class="btn btn-sm btn-info" title="Ver detalles">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                    <a href="empresas_editar.php?id=<?php echo $empresa['id']; ?>" class="btn btn-sm btn-warning" title="Editar">
                                                        <i class="bi bi-pencil"></i>
                                                    </a>
                                                    <?php if ($empresa['total_licencias'] == 0 && $empresa['total_usuarios'] == 0): ?>
                                                    <a href="#" class="btn btn-sm btn-danger" title="Eliminar" 
                                                       onclick="confirmarEliminar(<?php echo $empresa['id']; ?>, '<?php echo htmlspecialchars($empresa['nombre']); ?>')">
    ...
    <script>
        function confirmarEliminar(id, nombre) {
            ...
            document.getElementById('btnEliminar').href = 'empresas.php?eliminar=' + id;
