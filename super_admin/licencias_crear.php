...
try {
    ...
    $stmt = $conn->query("SELECT id, nombre, duracion_dias FROM tipo_licencia ORDER BY nombre");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $conn->prepare("SELECT COUNT(*) FROM licencias WHERE id_empresa = ? AND id_tipo_licencia = ? AND estado = 'Activa'");
        $stmt->execute([$id_empresa, $id_tipo_licencia]);
    ...
    if (empty($errores)) {
        try {
            $stmt = $conn->prepare("INSERT INTO licencias (id_tipo_licencia, fecha_inicio, id_empresa) VALUES (?, ?, ?)");
            $stmt->execute([$id_tipo_licencia, $fecha_inicio, $id_empresa]);
            
            $id_licencia = $conn->lastInsertId();
            
            $stmt = $conn->prepare("SELECT codigo_licencia FROM licencias WHERE id = ?");
...
<body>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="id_tipo_licencia" name="id_tipo_licencia" required onchange="actualizarDuracion()">
                                        ...
                                        <option value="<?php echo $tipo['id']; ?>" data-duracion="<?php echo $tipo['duracion_dias']; ?>" <?php echo ($id_tipo_licencia == $tipo['id']) ? 'selected' : ''; ?>>
                                            <?php echo htmlspecialchars($tipo['nombre']); ?> (<?php echo $tipo['duracion_dias']; ?> días)
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
    ...
    <script>
        ...
        document.getElementById('fecha_inicio').addEventListener('change', actualizarDuracion);
    </script>
</body>
</html>